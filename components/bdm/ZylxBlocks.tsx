...
function CommissionChart({ block }: { block: Extract<ZylxBlock, { type: 'chart' }> }) {
  ...
  const max = Math.max(1, ...block.series.flatMap((series) => series.points.map((point) => point.amount.minor)));
  const colours = ['bg-positive', 'bg-gold', 'bg-ink'];
  return <section className="rounded-panel border border-gold-line bg-white/65 p-4"><p className="text-sm font-bold text-ink">{block.title}</p><div className="mt-4 flex h-40 items-end gap-3 overflow-x-auto">{labels.map((label) => <div key={label} className="flex h-full min-w-12 flex-1 flex-col items-center justify-end"><div className="flex h-full items-end gap-1">{block.series.map((series, index) => { const amount = series.points.find((point) => point.x === label)?.amount; const value = amount?.minor ?? 0; return <div key={series.label} className={`w-4 rounded-t ${colours[index % colours.length]}`} style={{ height: `${Math.max(2, value / max * 100)}%` }} title={`${series.label}: ${amount?.display ?? formatMinor(value, block.currency || 'CAD', { showMinor: true })}`} />; })}</div><span className="mt-2 text-[11px] text-muted">{label.slice(5)}</span></div>)}</div><div className="mt-3 flex flex-wrap gap-4 text-xs text-muted">{block.series.map((series, index) => <span key={series.label}><span className={`mr-1 inline-block h-2.5 w-2.5 rounded-sm ${colours[index % colours.length]}`} />{series.label}</span>)}</div></section>;
}
